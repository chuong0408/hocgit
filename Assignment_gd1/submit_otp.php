...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($error == "") {
            $check_user = "SELECT * FROM khachhang WHERE email = ? AND OTP = ? AND OTP_time > NOW()";
            $result = $db_util->getOne($check_user, [$_POST['email'], $_POST['otp']]);

            if (!$result) {
                $error = "OTP đã hết hạn hoặc không hợp lệ.";
            } else {
                header("location: reset.php?id=" . urlencode($_POST['email']));
                exit();
            }
        }
    }
